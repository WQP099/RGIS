...
def getHtml(url_amap):  # <---获取网页信息--->

    page = urllib.request.urlopen(url_amap)  # 访问网页

    data = page.readline()  # 读入数据

    data_dic = json.loads(data,encoding="utf-8")  # 转换成python->字典
...
def getStaticAmap(lonlat_str, str_city_center):  # <---获取静态高德地图--->
    ...
    url = r'http://restapi.amap.com/v3/staticmap?location=%s&zoom=10&size=1024*768&key=dbebd5e43cae136f87e96d0c9b57f30d'
...
if __name__ == '__main__':  # <---主程序--->
    ...
    lonlat_str = "121.472644,31.231706"  # 定义空白定符串
    ...
    #     df_3 = df.ix[i, 5]  # 处理量
    #
    #     amap = r'http://restapi.amap.com/v3/geocode/geo?address=%s&city=上海&output=JSON&key=dbebd5e43cae136f87e96d0c9b57f30d'
    #
    #     url_amap = amap % df_2
    #
    ...
    # print df                                       #观察一下数据
    amap = r'http://webapi.amap.com/maps?v=1.4.6&key=dbebd5e43cae136f87e96d0c9b57f30d'

    #url_amap = amap % df_2
    #getLocation = getHtml(amap)
    ...
    #df.to_csv('WWT_lonlat.csv', index=False, sep=',', encoding='utf-8')  #